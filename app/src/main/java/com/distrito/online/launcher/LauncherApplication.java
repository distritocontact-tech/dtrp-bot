 ...
 * processo continuou vivo). Nesse caso, mesmo que a última tela visível
 * fosse a de login/connect, queremos reiniciar o fluxo pela tela de
 * carregamento (EntryActivity) — não simplesmente retomar de onde parou.
 *
...
public class LauncherApplication extends Application implements Application.ActivityLifecycleCallbacks {
    ...
    private boolean shouldRestartFromLoading = false;
    ...
    /**
     * Consome (lê e reseta) a flag de "precisa reiniciar pela tela de
     * carregamento". Chamado pelas Activities ao criar.
     */
    public boolean consumeShouldRestartFromLoading() {
        boolean value = shouldRestartFromLoading;
        shouldRestartFromLoading = false;
        return value;
    }
